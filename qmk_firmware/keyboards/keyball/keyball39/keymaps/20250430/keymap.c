...
void pointing_device_task(void) {
    ...
    int sensitivity_threshold = 5; // ZMKの 'tick' に相当する概念
    ...
    // <<== レイヤー3の場合の特別な処理（ここでは何もしないことで、後続のデフォルト処理に任せる）
    //else if (layer == 3) {
    ...
    // <<== その他のレイヤーの処理はそのまま
        ...
        // レイヤー5およびレイヤー3以外のレイヤーでは、デフォルトのマウス/スクロール動作を行わせます。
        // mouse_report の移動量は修正されていないため、後続の pointing_device_set_report で
        // 通常のマウス/スクロールとして処理されます。
        // ここで特別な処理は不要です。
    }
